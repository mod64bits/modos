...
class Chamado(models.Model):
    ...
    solicitante = models.ForeignKey(
        Usuario, on_delete=models.CASCADE, related_name="chamados_abertos"
    )
    setor = models.ForeignKey(Setor, on_delete=models.SET_NULL, null=True, blank=True)

    # O que é
    categoria = models.ForeignKey(
        CategoriaServico, on_delete=models.SET_NULL, null=True
    )
    ...
    # Solução
    relatorio_tecnico = models.TextField(
        "Laudo/Solução", blank=True, help_text="Descreva o que foi feito."
    )
    ...
    def nome_tecnico(self):
        ...
        1. Se tem técnico -> Nome do Técnico.
        2. Se foi CANCELADO e não tem técnico -> Nome do Solicitante.
        3. Padrão -> "--"
        """
        # 1. Tenta pegar o técnico atribuído
        if self.tecnico_atribuido:
            return (
                self.tecnico_atribuido.get_full_name()
                or self.tecnico_atribuido.username
            )

        # 2. Se não tem técnico e foi cancelado, assume-se que foi o solicitante
        if self.status == "CANCELADO":
            if self.solicitante:
                return self.solicitante.get_full_name() or self.solicitante.username
            return "Solicitante Desconhecido"

        # 3. Se não tem técnico e não foi cancelado (ex: Aberto)
        return "--"
    ...
    def save(self, *args, **kwargs):
        ...
        # 2. Lógica de Data de Fechamento
        if self.status in ["RESOLVIDO", "CANCELADO"] and not self.fechado_em:
            self.fechado_em = timezone.now()

        elif (
            self.status in ["ABERTO", "EM_ATENDIMENTO", "AGUARDANDO_PECA"]
            and self.fechado_em
        ):
            self.fechado_em = None

        # 3. Lógica de Transferência de Técnico (Log)
        if self.pk:
            try:
                old_instance = Chamado.objects.get(pk=self.pk)
                if old_instance.tecnico_atribuido != self.tecnico_atribuido:
                    old_name = (
                        old_instance.tecnico_atribuido.username
                        if old_instance.tecnico_atribuido
                        else "Ninguém"
                    )
                    new_name = (
                        self.tecnico_atribuido.username
                        if self.tecnico_atribuido
                        else "Ninguém"
                    )
