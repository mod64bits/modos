...
class Chamado(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # 2. Lógica de Data de Fechamento (A CORREÇÃO ESTÁ AQUI)
        # Se o status mudou para RESOLVIDO ou CANCELADO e ainda não tem data, preenche agora.
        if self.status in ['RESOLVIDO', 'CANCELADO'] and not self.fechado_em:
            self.fechado_em = timezone.now()
        
        # Opcional: Se reabrir o chamado, limpa a data de fechamento
        elif self.status in ['ABERTO', 'EM_ATENDIMENTO', 'AGUARDANDO_PECA'] and self.fechado_em:
