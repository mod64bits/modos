...
def enviar_email_chamado_task(chamado_id, created, log_id):
    ...
    admins_tecnicos = Usuario.objects.filter(is_staff=True, is_active=True).exclude(email__isnull=True).exclude(email="")
    ...
    texto_intro = "Um novo chamado foi aberto com sucesso no sistema e encaminhado para a equipa técnica." if created else "Ocorreu uma atualização, comentário ou alteração de status no chamado."
    link_chamado = f"{dominio_sistema}/orders/{instance.id}/"
    tecnico_nome = instance.tecnico_atribuido.get_full_name() if instance.tecnico_atribuido else "Aguardando Atribuição"
...
def enviar_pdf_chamado_email_task(chamado_id):
    ...
    try:
        chamado = Chamado.objects.select_related('solicitante', 'tecnico_atribuido', 'empresa').get(id=chamado_id)
    except Chamado.DoesNotExist:
        return
    ...
    if not destinatarios:
        logger.warning(f"O solicitante da O.S. #{chamado.numero} não possui e-mail registrado.")
        return
    ...
    try:
        email.send()
        logger.info(f"PDF da O.S. #{chamado.numero} enviado com sucesso para {destinatarios}.")
    except Exception as e:
